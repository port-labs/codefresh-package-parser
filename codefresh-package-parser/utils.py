...
def validate_and_load_env_vars():
    ...
    PACKAGE_FILTER_STRING = os.getenv('PACKAGE_FILTER_STRING', None)

    result = tuple((REPO_URL, ACCESS_TOKEN, PORT_CLIENT_ID, PORT_CLIENT_SECRET, PACKAGE_MANAGER, PACKAGES_FILE_PATH, PACKAGE_FILTER_STRING))
    for var in result:
        if var is None:

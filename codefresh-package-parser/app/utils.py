...
from app.consts import SUPPORTED_PACKAGE_MANAGERS
...
def validate_and_load_env_vars():
    ...
    GIT_PROVIDER_USERNAME = os.getenv('GIT_PROVIDER_USERNAME')
    GIT_PROVIDER_APP_PASSWORD = os.getenv('GIT_PROVIDER_APP_PASSWORD')
    ...
    result = tuple((REPO_URL, GIT_PROVIDER_USERNAME, GIT_PROVIDER_APP_PASSWORD, PORT_CLIENT_ID, PORT_CLIENT_SECRET, PACKAGE_MANAGER,
                    PACKAGES_FILE_FILTER, INTERNAL_PACKAGE_FILTERS))
    for var in result:
        if var is None:
            logger.error(f'One of the required parameters - REPO_URL, GIT_PROVIDER_USERNAME, GIT_PROVIDER_APP_PASSWORD, PORT_CLIENT_ID, PORT_CLIENT_SECRET, PACKAGE_MANAGER, PACKAGES_FILE_FILTER, INTERNAL_PACKAGE_FILTERS is missing')
            exit(1)
